...
<div class="bg-white rounded-2xl border border-slate-100 overflow-hidden table-responsive">
    <table class="w-full text-sm">
        ...
        <tr class="hover:bg-slate-50/50 transition">
            ...
            @php
                $statusLabels = ['aktif'=>'Aktif','tidak_aktif'=>'Tidak Aktif','lulus'=>'Lulus','keluar'=>'Keluar'];
                $statusColors = ['aktif'=>'bg-emerald-50 text-emerald-600','tidak_aktif'=>'bg-amber-50 text-amber-600','lulus'=>'bg-blue-50 text-blue-600','keluar'=>'bg-red-50 text-red-500'];
            @endphp
            <td class="px-5 py-3.5"><span class="px-2 py-1 rounded-full text-xs font-medium {{ $statusColors[$student->status] ?? 'bg-slate-100 text-slate-500' }}">{{ $statusLabels[$student->status] ?? $student->status }}</span></td>
            <td class="px-5 py-3.5">
                <div class="flex gap-1">
                    ...
                    <button data-edit-id="{{ $student->id }}" data-edit-nis="{{ $student->nis }}" data-edit-nisn="{{ $student->nisn }}" data-edit-nama="{{ $student->nama_lengkap }}" data-edit-jk="{{ $student->jk }}" data-edit-class="{{ $student->class_id }}" data-edit-status="{{ $student->status }}" class="p-1.5 rounded-lg hover:bg-accent-50 text-slate-400 hover:text-accent js-edit-btn" title="Edit"><i data-lucide="pencil" class="w-4 h-4"></i></button>
                    <form method="POST" action="{{ route('master.students.delete', $student->id) }}" onsubmit="event.preventDefault(); showConfirm('Hapus permanen siswa ini? Akun user juga akan ikut terhapus.', 'Hapus Siswa', 'Ya, Hapus', () => this.submit());" class="inline">
        ...
        <form id="studentForm" method="POST" action="{{ route('master.students.store') }}" class="p-5 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="jk" id="sJk" required class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-accent-200">
...
function openModal() {
    ...
    document.getElementById('sNama').value=''; document.getElementById('sJk').value='';
    document.getElementById('sClass').value=''; document.getElementById('sStatus').value='aktif';
    document.getElementById('studentModal').classList.add('show');
}
function editStudent(id,nis,nisn,nama,jk,classId,status){
    ...
    document.getElementById('sNama').value=nama; document.getElementById('sJk').value=jk;
    document.getElementById('sClass').value=classId; document.getElementById('sStatus').value=status;
    document.getElementById('studentModal').classList.add('show');
}
...
function openDetailModal(studentId) {
    ...
    .then(function(res) {
        ...
        var statusColorMap = {aktif:'bg-emerald-50 text-emerald-600',tidak_aktif:'bg-amber-50 text-amber-600',lulus:'bg-blue-50 text-blue-600',keluar:'bg-red-50 text-red-500'};
        var statusLabelMap = {aktif:'Aktif',tidak_aktif:'Tidak Aktif',lulus:'Lulus',keluar:'Keluar'};
        var statusLabel = statusLabelMap[d.status] || d.status;
        var statusClass = statusColorMap[d.status] || 'bg-slate-100 text-slate-500';
        ...
        content.innerHTML =
            '<div class="flex items-start gap-4 mb-5">' +
                ...
                '<div>' +
                    ...
                    '<span class="inline-block mt-1 px-2 py-0.5 rounded-full text-xs font-medium '+statusClass+'">'+statusLabel+'</span>' +
                '</div>' +
            '</div>' +
...
document.addEventListener('DOMContentLoaded',function(){
    document.querySelector('table').addEventListener('click',function(e){
        ...
        if (editBtn) {
            editStudent(editBtn.dataset.editId, editBtn.dataset.editNis, editBtn.dataset.editNisn, editBtn.dataset.editNama, editBtn.dataset.editJk, editBtn.dataset.editClass, editBtn.dataset.editStatus);
            return;
        }
