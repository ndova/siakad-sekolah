...
Route::middleware(['auth', 'role:' . implode(',', Role::internalRoles())])
    ...
    ->group(function () {
        ...
        Route::prefix('staff')->group(function () {
            // Staff CRUD (admin-only, defined above)

            // Absensi Pegawai (shared — guru bisa akses)
            Route::get('/attendance', [StaffController::class, 'attendanceGrid'])->name('staff.attendance.grid');
            Route::post('/attendance/store', [StaffController::class, 'storeAttendance'])->name('staff.attendance.store');
            Route::post('/attendance/bulk', [StaffController::class, 'bulkAttendance'])->name('staff.attendance.bulk');
            Route::get('/attendance/recap', [StaffController::class, 'attendanceRecap'])->name('staff.attendance.recap');
            Route::get('/attendance/export', [StaffController::class, 'exportAttendance'])->name('staff.attendance.export');
            Route::get('/attendance/import', [StaffController::class, 'importForm'])->name('staff.attendance.import');
            Route::post('/attendance/import', [StaffController::class, 'importAttendance'])->name('staff.attendance.import');
        });
